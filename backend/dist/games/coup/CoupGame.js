...
class CoupGame {
    ...
    createDeck() {
        const cards = [];
        const cardTypes = ["Duke", "Assassin", "Captain", "Ambassador", "Contessa"];
    ...
    initGame(roomId, players) {
        ...
        const coupPlayers = players.map((p) => {
    ...
    isValidTarget(targetId, state, selfId) {
    ...
    resolveChallenge(roomId, state, claimedPlayerId, challengerId) {
    ...
    getRequiredCardForAction(actionType) {
        switch (actionType) {
    ...
    resolvePendingAction(roomId, state) {
    ...
    loseInfluence(roomId, state, targetId) {
        const target = state.players.find((p) => p.playerId === targetId);
    ...
    loseCard(roomId, state, playerId, chosenCard) {
