...
const CoupGame_1 = require("./CoupGame");
describe("CoupGame", () => {
    let game;
    let players;
    let state;
    beforeEach(() => {
        game = new CoupGame_1.CoupGame();
        players = [
            {
    ...
    test("INCOME should give +1 coin and advance turn", () => {
        const action = { type: "INCOME", playerId: "P1" };
    ...
    test("COUP should cost 7 coins and remove influence", () => {
        ...
        const action = { type: "COUP", playerId: p1.playerId, payload: { targetId } };
        game.handleAction("room1", action, state);
    ...
    test("ASSASSINATE should cost 3 coins and set pendingAction", () => {
        ...
        p1.coins = 3;
        const targetId = state.players[1].playerId;
        const action = { type: "ASSASSINATE", playerId: p1.playerId, payload: { targetId } };
        game.handleAction("room1", action, state);
        expect(p1.coins).toBe(0);
    ...
    test("STEAL should set pendingAction", () => {
        const action = { type: "STEAL", playerId: "P1", payload: { targetId: "P2" } };
        game.handleAction("room1", action, state);
        expect(state.pendingAction).toEqual({
            type: "STEAL",
            fromPlayerId: "P1",
    ...
    test("EXCHANGE should set pendingAction", () => {
        const action = { type: "EXCHANGE", playerId: "P1" };
        game.handleAction("room1", action, state);
        expect(state.pendingAction).toEqual({
            type: "EXCHANGE",
            fromPlayerId: "P1"
    ...
    test("BLOCK should attach blockedBy to pendingAction", () => {
        ...
        const action = { type: "BLOCK", playerId: "P2" };
        game.handleAction("room1", action, state);
    ...
    test("Player should lose influence and possibly die", () => {
        ...
        // Lose influence
        game.loseInfluence(state, targetId);
