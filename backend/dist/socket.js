...
function emitError(socket, message, errorType = "error") {
    socket.emit("errorMessage", { type: errorType, message });
}
function emitRoomUpdate(io) {
    ...
    emitRoomUpdate.timeoutId = setTimeout(() => {
        io.emit("rooms:update", (0, rooms_1.getPublicRoomsSummary)());
    }, 100);
}
emitRoomUpdate.timeoutId = null;
    ...
    const sanitized = {
        ...
        players: state.players.map((p) => {
            ...
            return {
                ...
                ...(p.influence && {
                    influence: p.influence.map((card) => !card.revealed ? card : "UNKNOWN")
                })
            };
        }),
...
function sanitizeStateForAll(state) {
...
function initSocket(io) {
    ...
    const coup = new CoupGame_1.CoupGame();
    gameManager.registerGame(coup);
    coup.onEvent = (roomId, event, payload) => {
    ...
    io.on("connection", (socket) => {
        ...
        socket.emit("rooms:update", (0, rooms_1.getPublicRoomsSummary)());
        socket.on("getRooms", (ack) => {
            safeAck(ack, (0, rooms_1.getAllRooms)());
        });
        socket.on("createRoom", async (payload, ack) => {
            try {
                const { roomName, isPrivate = false, password, playerName, maxPlayers } = payload;
                const room = (0, rooms_1.createRoom)(roomName, isPrivate, password ?? null, maxPlayers);
                const player = (0, rooms_1.addPlayerToRoom)(room.roomId, socket.id, playerName, undefined, true);
                socket.join(room.roomId);
                const full = (0, rooms_1.getFullRoomData)(room.roomId);
                // Emit updates
                emitRoomUpdate(io);
        ...
        socket.on("joinRoom", async (payload, ack) => {
            try {
                ...
                const player = (0, rooms_1.addPlayerToRoom)(roomId, socket.id, playerName, playerId, false);
                if (!player) {
                ...
                socket.join(roomId);
                const full = (0, rooms_1.getFullRoomData)(roomId);
        ...
        socket.on("getCurrentPlayers", (payload, ack) => {
            try {
        ...
        socket.on("leaveRoom", (payload, ack) => {
            try {
                ...
                if (playerId) {
                    (0, rooms_1.removePlayerById)(roomId, playerId);
                }
                else {
                    (0, rooms_1.markPlayerDisconnected)(roomId, socket.id);
                }
                socket.leave(roomId);
                emitRoomUpdate(io);
                const full = (0, rooms_1.getFullRoomData)(roomId);
        ...
        socket.on("reconnectToRoom", (payload, ack) => {
            try {
                ...
                if (existingPlayer) {
                    ...
                    player = (0, rooms_1.addPlayerToRoom)(roomId, socket.id, playerName ?? "reconnected", playerId, false);
                    if (!player) {
        ...
        socket.on("game:start", async ({ roomId, gameId }, ack) => {
            try {
                const room = (0, rooms_1.getRoom)(roomId);
                if (!room) {
            ...
            try {
                ...
                if (updatedState && updatedState.players) {
                    ...
                    updatedState.players.forEach((p) => {
                        ...
                        if (targetSocket) {
                            io.to(targetSocket).emit("game:state", sanitizeStateForPlayer(updatedState, p.playerId));
                        }
                    });
                }
        ...
        socket.on("coup:loseCardChoice", (payload, ack) => {
            try {
                const { roomId, action } = payload;
                gameManager.handleAction(roomId, action);
                const updatedState = gameManager.getGameState(roomId);
